...
module.exports = (db) => {
  router.get("/", (req, res) => {
    ...
    db.query(
      `SELECT beverages.id, name, description, price, thumbnail_url, posted_at, beverages.is_available, sold_at, seller_id FROM beverages JOIN categories ON category_id = categories.id WHERE categories.type = 'Wine';`
    )
      .then((data) => {
